...
export function createCommands(getView: () => EditorView | null): Commands {
  ...
  const commands: Commands = {
    ...
    addSlide: (position = 'end') => exec(() => {
      ...
      if (!slideType || !rowType || !columnType || !paragraphType) return false;
      
      // Create empty slide: slide > row > column > paragraph
      const emptyParagraph = paragraphType.create();
      const emptyColumn = columnType.create(null, emptyParagraph);
